...
const ITEM_ICONS = {
  ...
  coal:'�ite', torch:'🔦', ender_pearl:'🟣', blaze_rod:'🥢',
...
async function loadPlayerDetail(uuid) {
  ...
  try {
    ...
    const [dataRes, statsRes] = await Promise.allSettled([
      state.playerDataCache[uuid] || api('GET', `/api/containers/${state.currentContainer.Id}/player-data/${uuid}?type=${state.currentContainer.serverType}`),
      state.playerStatsCache[uuid] || api('GET', `/api/containers/${state.currentContainer.Id}/player-stats/${uuid}?type=${state.currentContainer.serverType}`).catch(() => null),
    ]);
    ...
    if (!pdata) {
      panel.innerHTML = '<div class="pd-error">Could not load player data. The .dat file may be corrupted or inaccessible.</div>';
      return;
    }
...
document.getElementById('modal-confirm').addEventListener('click', confirmAddPlayer);
document.getElementById('modal-input').addEventListener('keydown', e => { if (e.key === 'Enter') confirmAddPlayer(); });
